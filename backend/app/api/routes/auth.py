...
async def register(payload: RegisterRequest, service: AuthService = Depends(get_auth_service)) -> RegisterResponse:
    """Crée un utilisateur puis envoie l'email de confirmation si nécessaire."""
...
async def confirm_email(token: str, service: AuthService = Depends(get_auth_service)) -> ConfirmEmailResponse:
    """Confirme un email à partir d'un token unique."""
...
async def resend_confirmation(
    ...
    """Rengendre et envoie un token de confirmation pour un email non confirmé."""
    await service.resend_confirmation_email(payload.email)
    await service.session.commit()
    return ResendConfirmationResponse(message="Email de confirmation renvoyé.")
...
async def forgot_password(
    ...
    """Déclenche une demande de reset mot de passe."""
...
async def reset_password(
    ...
    """Applique un nouveau mot de passe après validation du token de reset."""
...
async def login(
    ...
    """Authentifie l'utilisateur (TOTP si actif) et émet les tokens."""
    try:
...
async def refresh_tokens(payload: RefreshRequest, service: AuthService = Depends(get_auth_service)) -> AuthSession:
    """Recrée un couple de tokens à partir d'un refresh valide."""
...
async def logout(payload: RefreshRequest, service: AuthService = Depends(get_auth_service)) -> LogoutResponse:
    """Révoque un refresh token et sa session associée."""
